...
class ThongTinSinhVienController extends Controller
{
    ...
    protected function chiTietDiem($id, Content $content) {
        return $content
            ->header('Điểm')
            ->description('Danh sách điểm')
            ->body($this->gridDiem($id));
    }

    protected function gridDiem($id) {
        $grid = new Grid(new KetQuaDangKy());

        $grid->model()->where('id_sv',$id)->orderByDesc('id_dot_dang_ky');
        ...
        $grid->column('Số tín chỉ hiện tại')->display(function () use ($id){
            $idMonHoc = KetQuaDangKy::where('id_sv', $id)->pluck('id_mon_hoc');
